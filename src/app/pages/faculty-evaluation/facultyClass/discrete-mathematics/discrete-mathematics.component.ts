...
export class DiscreteMathematicsComponent {
  ...
  generalObservations =
    this.evaluationData[2]?.criteria?.filter((item: any) => item.question) ??
    [];

  // Extract ON SYNCHRONOUS SESSION questions only
  syncSession =
    this.evaluationData[2]?.criteria?.find(
      (item: any) => item.title === "ON SYNCHRONOUS SESSION"
    )?.criteria ?? [];

  get averageScore(): number {
    if (!this.syncSession.length) return 0;
    const total = this.syncSession.reduce(
